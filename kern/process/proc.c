...
alloc_proc(void)
{
    ...
    if (proc != NULL)
    {
        ...
        //LAB8 2310675 : (update LAB6 steps)
        /*
         * below fields(add in LAB6) in proc_struct need to be initialized
...
void proc_run(struct proc_struct *proc)
{
    // LAB4:填写你在lab4中实现的代码
        ...
        */
    //LAB8 2310675 : (update LAB4 steps)
      /*
       * below fields(add in LAB6) in proc_struct need to be initialized
...
int do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf)
{
    ...
    ret = -E_NO_MEM;
    // LAB8:EXERCISE2 2310675  HINT:how to copy the fs in parent's proc_struct?
    // LAB4:填写你在lab4中实现的代码
    /*
     * Some Useful MACROs, Functions and DEFINEs, you can use them in below implementation.
     * MACROs or Functions:
    ...
    // lab8:2310675 步骤1：分配并初始化 proc_struct
    if ((proc = alloc_proc()) == NULL) {
        goto fork_out;
    }

    // lab8:2310675（LAB5 更新）建立父子关系
    proc->parent = current;
    assert(current->wait_state == 0);

    // lab8:2310675 步骤2：为子进程分配内核栈
    if ((ret = setup_kstack(proc)) != 0) {
        goto bad_fork_cleanup_proc;
    }

    // lab8:2310675 步骤3：复制（或共享）内存管理结构 mm
    ...
    // lab8:2310675 步骤4：设置线程上下文（trapframe/context）
    copy_thread(proc, stack, tf);

    // lab8:2310675 步骤5：分配 pid 并原子地插入全局链表
    bool intr_flag;
    local_intr_save(intr_flag);
    {
        proc->pid = get_pid();
    ...
    // lab8:2310675 步骤6：把子进程设置为可运行
    wakeup_proc(proc);

    // lab8:2310675 步骤7：父进程返回子进程 pid
    ret = proc->pid;
...
{
    /* LAB8:EXERCISE2 2310675  HINT:how to load the file with handler fd  in to process's memory? how to setup argc/argv?
     * MACROs or Functions:
    ...
    // lab8:2310675（1）创建新的 mm
    if ((mm = mm_create()) == NULL) {
        goto bad_mm;
    }

    // lab8:2310675（2）建立 pgdir（拷贝内核映射）
    if (setup_pgdir(mm) != 0) {
        goto bad_pgdir_cleanup_mm;
    }

    // (3) read and validate ELF header
    struct elfhdr elf;
    ...
    // (4) setup user stack vma
    if ((ret = mm_map(mm, USTACKTOP - USTACKSIZE, USTACKSIZE, VM_READ | VM_WRITE | VM_STACK, NULL)) != 0) {
        goto bad_cleanup_mmap;
    }

    // (4)(6) setup argc/argv in user stack (argv strings + argv pointer array)
    ...
    // (4) allocate user stack pages (no page-fault handler in this lab, must pre-alloc pages)
    ...
    // (6) copy argv strings and argv array into user stack
    ...
    // Close the executable file after it is loaded.
    sysfile_close(fd);
    return 0;
